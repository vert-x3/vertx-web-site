...
----
shared import io.vertx.ceylon.auth.jwt "3.2.2";
----
...
To create an instance of the provider you use `link:../../ceylondoc/vertx-auth-jwt//JWTAuth.type.html[JWTAuth]`. You specify the configuration
in a JSON object.
...
value provider = jwtAuth.create(vertx, config);
...
if ("paulo" == username && "super_secret" == password) {
  value token = provider.generateToken(JsonObject {
    sub = "paulo";
  }, JWTOptions());
