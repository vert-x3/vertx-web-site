...
You can't do much in Vert.x-land unless you can commune with a `link:../../ceylondoc/vertx-core//Vertx.type.html[Vertx]` object!
...
value vertx = vertx_.vertx(VertxOptions {
  workerPoolSize = 40;
});

----

The `link:../../ceylondoc/vertx-core//VertxOptions.type.html[VertxOptions]` object has many settings and allows you to configure things like clustering,
...
`link:../../ceylondoc/vertx-core//VertxOptions.type.html[VertxOptions]` object before creating the Vertx object.
...
It's done by calling `link:../../ceylondoc/vertx-core//Vertx.type.html#executeBlocking(io.vertx.core.Handler,%20boolean,%20io.vertx.core.Handler)[executeBlocking]` specifying both the blocking code to execute and a
...
vertx.executeBlocking((Future<Object> future) {
  ...
  value result = someAPI.blockingMethod("hello");
  future.complete(result);
}, (Object|Throwable res) {
...
If you don't care about ordering you can call `link:../../ceylondoc/vertx-core//Vertx.type.html#executeBlocking(io.vertx.core.Handler,%20boolean,%20io.vertx.core.Handler)[executeBlocking]`
...
Coordination of multiple asynchronous results can be achieved with Vert.x `link:../../ceylondoc/vertx-core//Future.type.html[futures]`.

`link:../../ceylondoc/vertx-core//CompositeFuture.object.html#all(io.vertx.core.Future,%20io.vertx.core.Future)[CompositeFuture.all]` takes several futures arguments (up to 6) and returns a future that is
_succeeded_ when all the futures are and _failed_ otherwise:

[source,ceylon]
----
import io.vertx.ceylon.core { future, compositeFuture } 

...

value httpServerFuture = future.future();
httpServer.listen(httpServerFuture.completer());

value netServerFuture = future.future();
netServer.listen(netServerFuture.completer());

compositeFuture.all(httpServerFuture, netServerFuture).setHandler((CompositeFuture|Throwable ar) {
  if (is CompositeFuture ar) {
    // All server started
  } else {
...
The handler returned by `link:../../ceylondoc/vertx-core//Future.type.html#completer()[completer]` completes the future.

`link:../../ceylondoc/vertx-core//CompositeFuture.object.html#any(io.vertx.core.Future,%20io.vertx.core.Future)[CompositeFuture.any]` takes several futures arguments (up to 6) and returns a future that is
...
value future1 = future.future();
value future2 = future.future();
compositeFuture.any(future1, future2).setHandler((CompositeFuture|Throwable ar) {
  if (is CompositeFuture ar) {
...
`link:../../ceylondoc/vertx-core//Future.type.html#compose(io.vertx.core.Handler,%20io.vertx.core.Future)[compose]` can be used for chaining futures:
...
value fut1 = future.future();
value fut2 = future.future();
...
If you want to deploy a verticle as a worker verticle you do that with `link:../../ceylondoc/vertx-core//DeploymentOptions.type.html#setWorker(boolean)[worker]`.
...
You can deploy a verticle using one of the `link:../../ceylondoc/vertx-core//Vertx.type.html#deployVerticle(io.vertx.core.Verticle)[deployVerticle]` method, specifying a verticle
...
You can also programmatically register and unregister verticle factories using `link:../../ceylondoc/vertx-core//Vertx.type.html#registerVerticleFactory(io.vertx.core.spi.VerticleFactory)[registerVerticleFactory]`
and `link:../../ceylondoc/vertx-core//Vertx.type.html#unregisterVerticleFactory(io.vertx.core.spi.VerticleFactory)[unregisterVerticleFactory]` if you wish.
...
[source,ceylon]
----
vertx.deployVerticle("com.mycompany.MyOrderProcessorVerticle", (String|Throwable res) {
  if (is String res) {
    print("Deployment id is: ``res``");
  } else {
...
Deployments can be undeployed with `link:../../ceylondoc/vertx-core//Vertx.type.html#undeploy(java.lang.String)[undeploy]`.
...
[source,ceylon]
----
vertx.undeploy(deploymentID, (Throwable? res) {
  if (!exists res) {
    print("Undeployed ok");
  } else {
...
`link:../../ceylondoc/vertx-core//DeploymentOptions.type.html#setIsolatedClasses(java.util.List)[isolatedClasses]`-  an entry can be a fully qualified
...
Extra classpath entries can also be provided with `link:../../ceylondoc/vertx-core//DeploymentOptions.type.html#setExtraClasspath(java.util.List)[extraClasspath]` so if
...
options.isolatedClasses = ArrayList { "com.mycompany.myverticle.*",  "com.mycompany.somepkg.SomeClass",  "org.somelibrary.*" };
...
If you are embedding Vert.x and you have finished with it, you can call `link:../../ceylondoc/vertx-core//Vertx.type.html#close()[close]` to close it
down.
...
To retrieve the context, use the `link:../../ceylondoc/vertx-core//Vertx.type.html#getOrCreateContext()[getOrCreateContext]` method:

[source, ceylon]
----
value context = vertx.getOrCreateContext();
...
[source, ceylon]
----
import io.vertx.ceylon.core { context_ = context } 

...

value context = vertx.getOrCreateContext();
if (context.isEventLoopContext()) {
  print("Context attached to Event Loop");
} else if (context.isWorkerContext()) {
  print("Context attached to Worker Thread");
} else if (context.isMultiThreadedWorkerContext()) {
  print("Context attached to Worker Thread - multi threaded worker");
} else if (!context_.isOnVertxThread()) {
  print("Context not attached to a thread managed by vert.x");
};
...
vertx.getOrCreateContext().runOnContext(() {
  print("This will be executed asynchronously in the same context");
});
...
`link:../../ceylondoc/vertx-core//Context.type.html#runOnContext(io.vertx.core.Handler)[runOnContext]`:
...
context.runOnContext(() {
  value hello = context.get("data");
});

----

The context object also let you access verticle configuration using the `link:../../ceylondoc/vertx-core//Context.type.html#config()[config]`
...
To set a timer to fire once you use `link:../../ceylondoc/vertx-core//Vertx.type.html#setTimer(long,%20io.vertx.core.Handler)[setTimer]` method passing in the delay and a handler

[source,ceylon]
----
value timerID = vertx.setTimer(1000, (Integer id) {
...
You can also set a timer to fire periodically by using `link:../../ceylondoc/vertx-core//Vertx.type.html#setPeriodic(long,%20io.vertx.core.Handler)[setPeriodic]`.
...
In this case, you should consider using `link:../../ceylondoc/vertx-core//Vertx.type.html#setTimer(long,%20io.vertx.core.Handler)[setTimer]` instead. Once your treatment has
finished, you can set the next timer.

[source,ceylon]
----
value timerID = vertx.setPeriodic(1000, (Integer id) {
...
To cancel a periodic timer, call `link:../../ceylondoc/vertx-core//Vertx.type.html#cancelTimer(long)[cancelTimer]` specifying the timer id. For example:
...
For example if you have deployed a verticle which creates a `link:../../ceylondoc/vertx-core//net/NetServer.type.html[NetServer]` which provides
`link:../../ceylondoc/vertx-core//net/NetSocket.type.html[NetSocket]` instances in it's handler, then it's best to always access that socket instance
...
`link:../../ceylondoc/vertx-core//VertxOptions.type.html#setClusterManager(io.vertx.core.spi.cluster.ClusterManager)[clusterManager]`.
...
value logger = LoggerFactory.getLogger(className);
