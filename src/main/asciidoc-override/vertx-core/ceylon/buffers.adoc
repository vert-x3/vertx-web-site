...
Buffers can create by using one of the static `link:../../ceylondoc/vertx-core//buffer/Buffer.object.html#buffer()[Buffer.buffer]` methods.
...
value buff = buffer.buffer("some string");
...
value buff = buffer.buffer("some string", "UTF-16");
...
value buff = buffer.buffer(10000);
...
variable value i = 0;
while (i < buff.length()) {
  print("int value at ``i`` is ``buff.getInt(i)``");
  4};
...
value buff = buffer.buffer(128);
value pos = 15;
buff.setUnsignedByte(pos, 200);
print(buff.getUnsignedByte(pos));
...
Use `link:../../ceylondoc/vertx-core//buffer/Buffer.type.html#length()[length]` to obtain the length of the buffer.
The length of a buffer is the index of the byte in the buffer with the largest index + 1.

=== Copying buffers

Use `link:../../ceylondoc/vertx-core//buffer/Buffer.type.html#copy()[copy]` to make a copy of the buffer

=== Slicing buffers

A sliced buffer is a new buffer which backs onto the original buffer, i.e. it does not copy the underlying data.
Use `link:../../ceylondoc/vertx-core//buffer/Buffer.type.html#slice()[slice]` to create a sliced buffers
