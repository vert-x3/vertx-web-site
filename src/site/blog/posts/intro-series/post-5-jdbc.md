...
This method also uses the `execute` method with an _INSERT_ statement. Once the the statement has been executed, we issue a query to retrieve the inserted row (in order to get the id generated by the database).

Let's have a look to the result provided to the handler passed to the `query` method. It gets a `ResultSet`, which contains the query result. Each row is a `JsonObject`.
